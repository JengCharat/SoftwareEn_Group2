...
| AC-3 | รหัสผ่านต้องไม่**มีคำใน word list ฝังอยู่** (substring match สำหรับคำที่ยาว >= 5 ตัว) | ป้องกัน `aaaaaa` x2 = `aaaaaaaaaaaa` และรูปแบบซ้ำอื่นๆ |
...
- `isCommonPassword(password)` — ตรวจสอบ 2 ระดับ:
  - **Exact match**: ทุกคำใน list (case-insensitive)
  - **Substring match**: เฉพาะคำที่ยาว >= 5 ตัว (ป้องกัน false positive คำสั้นอย่าง `god`, `red`)
...
- อัปเดต placeholder: `"อย่างน้อย 10 ตัวอักษร"`
- อัปเดต hint text: `"ต้องมีความยาวอย่างน้อย 10 ตัวอักษร และต้องไม่เป็นรหัสผ่านยอดนิยม (NCSC UK)"`
...
- `createUserSchema.password`:
  - เปลี่ยน `.min(8)` → `.min(10, "Password must be at least 10 characters (NCSC UK)")`
  - เพิ่ม `.refine(pw => !isCommonPassword(pw), { message: "Password is in the common brute-force word list (NCSC UK)..." })`
...
- Sprint: Sprint 3
- AI Declare (Claude Sonnet 4.6) : ใช้ในการออกแบบ word list, logic การตรวจสอบ (substring/exact match), Frontend validation, Backend Zod schema
